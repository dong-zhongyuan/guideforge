    ...
    if len(idx_tb) >= 5 and len(idx_f1f) >= 5:
        ...
        fig1f_holdout = {
            ...
            "extrapolation_note": "决策树预测上界=训练 y 最大值; 全部 %d 条 fig1f "
                "标签高于训练上界 %.3f → MAE 超阈主因是训练/验证子集活性范围不重叠"
                "(工具箱=精选强抑制功能子集 0.443-0.583, fig1f 对=多区饱和突变体 "
                "n_mut 3-6 抑制更弱 0.611-0.928; 同一 CRISPRi gfp 读数同一归一化, "
                "组成性标签越界)叠加树模型不可外推, 非排序能力失败; "
                "2026-09-11 文献复核更正: 旧表述'主图/附图图版间水平偏移'不准确, "
                "Han 2025 Fig.1 图注证实 fig1f 为序列表、活性同源 panel e/g 同测定; "
                "本用途(先验排序)的有效判据是秩一致性(Spearman 及其 CI)"
                % (n_out, y_tr.max()),
