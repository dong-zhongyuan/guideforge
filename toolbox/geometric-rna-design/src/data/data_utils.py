...
try:
    import cpdb
except ImportError:  # cpdb-protein 编译失败时的兜底(本服务器 gcc 过旧)
    cpdb = None
import numpy as np
import pandas as pd
import torch
...
from src.constants import FILL_VALUE, PURINES, PYRIMIDINES, RNA_ATOMS, RNA_NUCLEOTIDES
...
def pdb_to_tensor(
    ...
    if cpdb is not None:
        ...
        from biotite.structure.io import load_structure as _load
        _arr = _load(filepath)
        if hasattr(_arr, 'stack_depth'):
            _arr = _arr[0]
        df = pd.DataFrame({
            'record_type': 'ATOM',
            'atom_name': _arr.atom_name,
            'residue_name': _arr.res_name,
            'chain_id': _arr.chain_id,
            'residue_number': _arr.res_id,
            'insertion': getattr(_arr, 'ins_code', ''),
            'x_coord': _arr.coord[:, 0],
            'y_coord': _arr.coord[:, 1],
            'z_coord': _arr.coord[:, 2],
        })
...
def df_to_tensor(
    ...
    """Transforms a DataFrame of an RNA structure into a ``length x num_atoms x 3`` tensor.
