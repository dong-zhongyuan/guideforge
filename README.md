...
### Round-4 负面结果整改批次(§G, 2026-09-11; 判据预登记 docs/preregistration.md §G, 登记 d455bfe 先于全部评估)

- **G1 cross_pp 混杂归因强化**(`crrna_cross_decompose.py` 新增双协变量偏 Spearman + GC 分层置换): raw_coreDR@mature18 ρ=+0.546 在 GC+A 双控后仍 +0.475(LOO 稳定), 但 GC 分层置换 p=0.159 不显著, 组成匹配 z 口径维持 +0.17(分层 p=0.54)——**G1c 判读=部分支持**: raw 不可引用、z 口径无信号的处置不变, 双控数值入库(`data/cross_decomposition.json` verdict.G1c_preregistered)。
- **G2 选型器秩稳定量化**(`crrna_train_selector.py` rank_stability 块, bootstrap B=500 + Borda 树/Ridge 聚合): 池化排序首位 A1C 的 TOP1 入选频率仅 0.408(U5G+A18C 0.368)——**判读=首位弱稳定**; Borda 首位 A1U+A2C 与单模型不一致, 按预登记规则以 Borda 为报告口径。「数据增量敏感」定性结论获得逐条频率/秩 CI 的量化口径, 定位仍是文献先验排序不变。
- **G3 位点敏感性×保守性合并检验**(`crrna_dmytrenko_validate.py`, 置换校准 Fisher, 同步置换处理三口径相关性): v6 现役口径三方向 ρ=−0.426/−0.406/−0.363(均负), 合并 p=0.068——未达预登记 0.05, **维持「方向一致、单口径不显著」**, 合并口径数值入库。
- **G4 DeWeirdt 大库位点敏感性**(`scripts/crrna_deweirdt_position_sensitivity.py`, n_mut==1 子集, 坐标偏移 +1 断言校验): 两方向 ρ=−0.508/−0.508, 置换 p=0.066/0.065——方向与 G3 一致但边缘不显著; 诚实边界: DeWeirdt 为组合设计库, 单点突变子集稀疏(每位 1–3 条, 14/18 位有覆盖)。**第二条证据链方向一致、显著性不足, 如实报告**。
- **G5 Creutzburg 论文口径对齐重测**(`scripts/crrna_creutzburg_pk_caliber.py`): 原登记指标 P(茎|P1 强制)首跑发现构造性恒零(P1 与茎配对交叉互斥, 当日修订登记先于新指标评估); 修订口径 p_stem_paper = P(WC 茎完整) + exp(−ΔG_pk/RT) → ρ=+0.018(p=0.95), 分量 w_P1 ρ=−0.062——**P1 对齐口径亦阴性**: 未复现不能归因于 pre-crRNA 上下文与 P1 茎定义两处口径差, 剩余差异仅 n(14 vs ~25)与读图活性值。维持现表述, 该口径阴性追加记录。
- **G6 RRS 位置感知模型(升级)**(`scripts/crrna_rrs_position_model.py`, CRISPRpred 位置特征先例, LOCO 8 折): 位置⊗碱基 12 维 + 结构 5 维, LOCO Spearman **+0.714** ≥ 预登记阈 +0.3(消融: 仅位置 +0.677 / 仅结构 +0.121——证实 RRS 信号由位置承载)——**RRS 区候选从「按位置规则排除」升级为「位置感知模型打分」, 范围限定: 仅 Tian 同族 RRS 区(DR 5' 端 4nt)、仅相对排序**。负面清单 #7(Tian 外部验证≈0)由此闭环: 不是数据无信号, 是结构特征与该区位点机制不匹配, 换位置口径后同数据可预测。
- **G7 层级部分池化**(`scripts/crrna_hierarchical_pool.py`, 组随机截距+共享 Ridge 斜率, 交替最小二乘): DeWeirdt 带截距入池后其余 5 终点 LOEO 正值数=1(fig1g +0.07)<3 → **按预登记维持拒入**; 但 cis 终点层级 LOEO −0.429 较决策树口径 −0.692 改善, DeWeirdt 自身层级 LOEO −0.065——层级口径数值全量入库, 层级模型作为终点整合第二口径与决策树并存。

### Round-4 残余负面结果整改批次(§H, 2026-09-11; 判据预登记 docs/preregistration.md §H, 登记 2e79f11 先于全部评估)

§G 评估后仍为负的三条, 按文献先例(DeepCRISTL PMID 35758815 / Meinshausen-Bühlmann 2010 / Shah-Samworth 2013 / jbloomlab dms_variants / Haddox 2023)逐一上攻, 结果全部如实入库:

- **H1 cis 反预测 → 随机斜率层级模型+方向一致性门控**(同脚本新增 `h1_random_slopes` 块): b[g] 收缩常数 LAM_SLOPE=10, 组内斜率与共享斜率 cos<0 的终点权重减半并列名——**7 终点中 4 个方向冲突**(cis_end −0.238 / deweirdt_lfc127 −0.334 / rbs0 −0.151 / rbs33 −0.204, 即终点间结构关系本身异质, 这是 cis 反预测的机制层证据); cis 随机斜率 LOEO=−0.429, 未较随机截距口径改善 → **按预登记维持原值**, 判读: cis 反预测的本质是多终点方向异质而非模型缺斜率自由度, 根治靠 §F 湿数据重训。
- **H2 池化排序弱稳定 → 逐对胜率矩阵+稳定性选择判读**(`crrna_train_selector.py` rank_stability.h2_pairwise): 28 对逐对胜率(bootstrap B=500 秩向量, 并列计 0.5)——**稳健序(P≥0.9) 0/28, 倾向序(0.6–0.9) 9/28, 不可分辨 19/28**; Shah-Samworth 误差界(π=0.6) E[FP]≤14.46。按预登记维持弱稳定表述; 但矩阵给出一个可辩护的结构性读数: 不可分辨集中在头部候选之间(首位集团互相胜率 0.4–0.6), 而 **B_break_compensate 在全部 7 对中一致列末位**(P=0.62–0.71 倾向档)——「补偿臂破坏茎则排序垫后」这一方向是稳定的, 弱稳定的是头部候选间的相对次序而非整个排序。
- **H3 位点敏感性×保守性边缘 → 全量库加性潜表型口径**(同脚本新增 `h3_full_library_additive` 块): 废弃单点子集(n=31), 全量 35,879 条 Ridge(位置×替代碱基 one-hot, α=1.0)拟合加性效应, 位点敏感性=3 alt |效应|均值——**方向分裂**: LFC127 ρ=−0.107(p=0.70) / LFC128 ρ=+0.313(p=0.26) → 按预登记如实报告, G3/G4 维持「方向一致、边缘不显著」原表述, 新口径数值并列入库。解读边界: DeWeirdt 库两读数方向分裂提示该库加性口径不支持保守性-敏感性关联, 单点子集的方向一致可能受稀疏抽样影响——该关联目前的最佳证据仍是 v6 现役三口径(G3), 强度如实标注为边缘。
- 契约测试 `tests/test_round5_h_batch.py`(12 项, 锁判读文字与数值同向), 全量 127 passed。

### 同源迁移证据链补强(§I, 2026-09-11; 判据预登记 docs/preregistration.md §I, 登记 dc9bfa4 先于评估)

迁移链原有 L1 文献锚点(Dmytrenko 2023 ED Fig.2c Cas12a↔Cas12a2 DR 互换功能保持)/ L2 结构层(Protenix 界面恢复 0.835 + cas12a2 活性模板无假结)/ L3 序列层(11 同源 DR 比对); 本节补两个可计算环节(`scripts/crrna_transfer_domain.py` -> `data/transfer_domain.json`):

- **I1 适用域分析**: 候选(v6 全 pass 库 204 条 + 8 员族)在池化训练集(46×5)标准化空间的 5-NN 包络内占比 **45.3%(96/212)<95%** → 按预登记如实报告并追加适用域边界声明。细分: **8 员族(答辩核心候选)5/8 在包络内**, 越界 3 条 = A8C+U15G / A1G+U3A+A8G+U15C / U5G+A18C——恰为既有负面清单上的候选(通用性降级、排序翻转主角); 越界集中 ddG_dr/p_fold/spacer_up 三特征(bp_dist/cross_nt 零越界), 机制=文献训练集多为失活突变、v6 pass 库经结构保持过滤后系统性更温和。**迁移主张按适用域分层: 包络内候选=内插(含 5/8 员族), 包络外候选标注外推**。
- **I2 折叠系综同构**: 11 条同源 DR 配分系综逐列配对谱(18 比对列), Cas12a2×Cas12a 18 对两两 Spearman **中位 ρ=1.000(最低 0.976)**, 高于 Cas12a 内部对照中位 0.991 → **「两体系 DR 折叠系综同构」获支持**。诚实边界: 短茎谱高度定型(多数列接近 0/1), 本检验证明「DR 折叠行为同构」, 不单独构成活性迁移证据——迁移链强度=L1 实验锚点为主, L2/L3/I1/I2 为佐证, 终验=湿实验。
- 契约测试 `tests/test_round5_i_batch.py`(8 项), 全量 135 passed。
...
3. **AF3 独立对照层(2026-09-04 初版; 2026-09-05 格式勘误重发; 2026-09-10 官方本地层+主张降级, 策划案 V3 表1 口径)**: 立项时本机无 AF3 权重(需 DeepMind 审批)与 GPU 机时, 先生成 AlphaFold Server 就绪任务 13 套(WT + TOP-12, `data/af3_inputs/`, 生成器 `scripts/crrna_af3_input.py`, 蛋白/靶 RNA 与 Chai 矩阵同源; 逐任务文件 + 合并单文件 `GF_all13_alphafoldserver.json`)。**勘误两处**: ①旧版误用 AF3 开源代码库方言(dialect=alphafold3, 带链 id), 服务器只认 `alphafoldserver` 方言(顶层任务列表, 实体键 proteinChain/rnaSequence, 官方 server/README.md 口径), 已重发; ②旧版"服务器不接受自定义模板→恰好 template-free"的说法不成立——服务器**默认自动启用 PDB 模板**(8D4A/8D49 在库), 已在蛋白链强制 `useStructureTemplate=false`, 才是真正的 template-free 独立对照。提交操作与判读口径(预登记二分支: 界面恢复→独立佐证 / 界面低迷→主张撤回, 均完整可交付)见 `docs/af3_server_runbook.md`; 结果 zip 放入 `data/af3_results/` 后用 `scripts/crrna_af3_collect.py` 回填汇总(mock 包解析已测)。若 AF3 template-free 下 prot-crRNA 界面同样恢复, "界面可预测"才具备首个独立证据。**AF3 权重不可得的开源替代通道(2026-09-05)**: Protenix-v1(`protenix_base_default_v1.0.0`, ByteDance, Apache-2.0 代码+权重全开放, 368M 参数与 AF3 同规模、训练截止 2021-09-30 与 AF3 对齐, 基准在同等约束下达到/超过 AF3, bioRxiv 2026.02.05.703733)——13 套同一复合物输入已按 Protenix 方言转换(`data/protenix_inputs/`, 生成器 `scripts/crrna_protenix_input.py`, 逐链序列与 AF3 输入 13/13 一致; 不含 templates 字段即 template-free, 与 AF3 口径一致), 云端运行手册 `tools/protenix_cloud_runbook.md`(4090 级 GPU <2 小时跑完), 结果回填 `data/protenix_results/` 后与 Chai/AF3 做跨引擎一致性对照。**Protenix 13 折结果已回收并出对照(2026-09-09)**: 容器 A6000 批跑(2026-09-06, seed_101 × 5 sample)产物 13/13 完整回收, `scripts/crrna_af3_collect.py --protenix` 汇总为 `data/protenix_summary.json`——**预登记分支一触发: template-free 独立引擎下 WT prot-crRNA ipTM 0.835(5 sample sd 0.001), 全部 13 套 0.802–0.832 均 ≥0.6 可用阈, 界面恢复成立**; Chai 自模板 WT 参照 0.489, 两引擎界面恢复方向一致(数值差 0.35 属引擎间离散)。**"蛋白-crRNA 界面可预测"主张自此具备独立引擎佐证**(此前仅有 Chai 自模板单引擎证据); 变体对 WT 差量 -0.003~-0.033 且 WT 居首, 与 Chai 差量表方向一致, 但按预登记口径本层不用于骨架间排序主张。AF3 云端通道(alphafoldserver)**中止(2026-09-09, 用户决定)**: 提交需 Google 登录+逐任务手动上传, 自动化通道全部受阻(computer-use 会话锁/新版 Edge 禁止默认配置远程调试); 已提交的 GF_WT/GF_A1C 两任务在服务器队列自行运行, 结果 zip 若日后回收可作带 MSA 口径的官方单点抽查。**官方 AF3 本地层(2026-09-10, 权重获批后, 提交 4d69537)**: DeepMind 官方 `af3.bin.zst` 权重, alphafold3-3.0.4 开源推理, A6000 批跑同 13 套(template-free 且**无 MSA**——蛋白/RNA 均单序列自比对输入, 与 Protenix 协议同构; seed 0 × 5 diffusion samples; `data/af3_local_results/`, 汇总 `data/af3_local_summary.json`)——**WT prot-crRNA ipTM 0.252(sd 0.040), 低于 0.5-0.6 可用区间, 界面未恢复**。**主张现状(2026-09-10 起, 替代 09-09 旧表述): "蛋白-crRNA 界面可预测"从"已获独立引擎佐证"降级为"两引擎分歧、终裁未定"**——同无 MSA 协议下 Protenix 0.835(支持界面恢复) vs 官方 AF3 0.252(不支持), `engines_agree=false` 程序化对照已入库(`crrna_af3_collect.py --local` 的 `protenix_cross_engine` 字段); Protenix 层自身口径内的登记判读文本维持不变, 但**任何引用必须并列 AF3 分歧结果**; 终裁路径 = 带 MSA 口径(本地 AF3 带 MSA 重跑, 或回收 AlphaFold Server 在队 WT/A1C 两任务)。09-09 云端中止段"中止不削弱现有主张"一句自本条起废止。
